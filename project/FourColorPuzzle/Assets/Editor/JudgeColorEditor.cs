...
public class JudgeColorEditor : EditorWindow
{
	...
	[MenuItem("ComponentSettings/JudgeColor")]
	static void CreateWindow()
	{
		// ウインドウの取得.
		var Window = EditorWindow.GetWindow(typeof(JudgeColorEditor));
		// ウインドウの表示.
		Window.Show();
	}
	...
	void SetComponent()
	{
		...
		for (int i = 0; i < TargetGameObjectCount; i++)
		{
			...
			if (isAdd)
			{
				TargetGameObject[i].AddComponent<JudgeColor>();
				Debug.Log(TargetGameObject[i].name + "にJudgeColorを追加");
			}
			else
			{
				GameObject.DestroyImmediate(TargetGameObject[i].GetComponent<JudgeColor>());
				Debug.Log(TargetGameObject[i].name + "からJudgeColorを削除");
			}
		}
	}
